...
class Help:
    ...
    再例如您输入:
    >>> tag VOCALOID miku 10 1000
    就可以下载同时带有[VOCALOID]和[miku]标签的前[10]幅插画，并且人气高于[1000]。

    {Color.yellow}[提示] 如果您搜索的标签是热门标签，相比起用指令输入人气值，我更推荐您使用形如"10000users入り"的标签，它能够更快地找到符合条件的插画!{Color.end}
    例如您输入:
    >>> tag miku 10000users入り 10
    能够更快地找到人气值高于[10000]的[miku]插画。
    '''
